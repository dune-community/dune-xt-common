...
# cpf               Use comunity preview edition (links to libtbb_preview instead of libtbb). cpf is not available for
# the built-in version of the Intel Compiler, but see the note on debug mode above for a fix. allocator         Use
# TBB's scalable allocator (links to libtbbmalloc).
#
...
# search paths, otherwise we leave them in
...
if(TBB_DEBUG)
  ...
  set(tbb_debug_suffix "_debug") # TBB requires this additional compile definition when used in debug mode
  list(APPEND TBB_COMPILE_DEFINITIONS TBB_USE_DEBUG)
else()
  set(tbb_debug_suffix "")
endif()

# start looking for components We first look for component libraries, because the "special component" cpf actually
# replaces the standard libtbb
...
foreach(component ${TBB_FIND_COMPONENTS})
  ...
  elseif(component STREQUAL "allocator")
    find_tbb_library(VAR TBB_LIBTBBMALLOC NAME "tbbmalloc${tbb_debug_suffix}" DOC "Path to TBB malloc library")
    if(TBB_LIBTBBMALLOC)
...
if(NOT TBB_cpf_FOUND)
  find_tbb_library(VAR TBB_LIBTBB NAME "tbb${tbb_debug_suffix}" DOC "Path to TBB library")
  if(TBB_LIBTBB)
...
mark_as_advanced(TBB_LIBTBB_PREVIEW TBB_LIBTBB TBB_LIBTBBMALLOC)

include(CheckCXXSourceCompiles)
include(CMakePushCheckState)

# make sure this variable always exists; it is only used if we pick the internal TBB implementation from an Intel
# Compiler
...
if((NOT TBB_LIBTBB) AND ("${CMAKE_CXX_COMPILER_ID}" STREQUAL "Intel"))
  ...
  if(NOT TBB_DEBUG)
    ...
    check_cxx_source_compiles("${tbb_compile_source}" TBB_INTEL_COMPILER_INTERNAL_TBB)

    if(TBB_INTEL_COMPILER_INTERNAL_TBB) # yeah, success
      ...
      foreach(component ${TBB_FIND_COMPONENTS})
        ...
        elseif(component STREQUAL "allocator") # we'll check for this by trying to link against the library
          ...
          check_cxx_source_compiles("${tbb_compile_source}" TBB_INTEL_COMPILER_INTERNAL_TBBMALLOC)
...
check_cxx_source_compiles("${tbb_compile_source}" TBB_COMPILE_TEST)
cmake_pop_check_state()
